...
// строки в терминале шире столбцов примерно в два раза, и это нужно учитывать при отрисовке всяких симметричных вещей
const Y_SCALE_FACTOR: f32 = 0.5;

fn main() {
    // let mut x = 0.0;
    // while x <= 2.0 * PI {
    //     println!("{}: {}", x, angle_to_line_char(x + PI * 0.5));
    //     x += 0.1;
    // }
    // return;
    ...
    let ms_duration = Duration::from_millis(100);
...
impl RadiationWing {
    fn new(angle: f32, res: &TerminalResolution) -> Self {
        let (rows, cols) = res.get_rows_cols();
...
fn draw_rad_bounds(buff: &mut VirtualBuffer, a1: f32, a2: f32, r1: f32, r2: f32) {
    for a in [a1, a2].iter() {
        for r in [r1, r2].iter() {
            let (x, y) = rad_coords_to_cartesian(*a, *r);
            buff.set_px(x as i32, (y * Y_SCALE_FACTOR) as i32, Some('*'))
        }
    }
}
